...
const benefits = [
  { icon: HeartPulse, title: "Supports digestion", copy: "Live cultures keep your gut calm and happy every day." },
  { icon: Shield, title: "Boosts immunity", copy: "A daily sip helps your body's natural defences stay sharp." },
  { icon: Droplets, title: "Better nutrient absorption", copy: "Calcium and protein your body can actually put to work." },
  { icon: Snowflake, title: "Reduces bloating", copy: "Light, cold and easy — never heavy after you drink it." },
];
    ...
    <div>
      ...
      <section className="relative overflow-hidden bg-hero-gradient pb-28 pt-28 sm:pb-40 sm:pt-36">
        <div className="mx-auto grid max-w-6xl items-center gap-10 px-5 sm:px-8 md:grid-cols-2">
          <motion.div initial="hidden" animate="show" variants={staggerParent}>
            ...
            <motion.p variants={fadeUp} className="mt-4 max-w-md text-base text-muted-foreground sm:text-lg">
      ...
      <section className="mx-auto mt-24 max-w-6xl px-5 sm:px-8">
        <motion.div initial="hidden" whileInView="show" viewport={viewportOnce} variants={staggerParent}>
          ...
          <motion.ul variants={staggerParent} className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            {benefits.map((b) => (
              <motion.li
                key={b.title}
                variants={fadeUp}
